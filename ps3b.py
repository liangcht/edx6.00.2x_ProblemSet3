...
class Patient(object):
    ...
    def update(self):
        ...
        for u in self.getViruses()[:]:
            try :
                self.getViruses().append(u.reproduce(popDensity))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials) :
        ...
        for j in range(len(v_populations[:])) :
            v_populations[j] += patient.update()

    avg_v_population = [ x/float(numTrials) for x in v_populations]

    pylab.plot(range(300), avg_v_population, label = 'simulation without drug')
    pylab.ylabel('average size of the virus population')
    pylab.xlabel('time steps')
    pylab.title('Simple simulation without drugs')
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for u in self.getViruses()[:]:
            try :
                self.getViruses().append(u.reproduce(popDensity, self.getPrescriptions()))
            except NoChildException :
                pass
        return len(self.getViruses())
